...
def gerenciamento_tarefas_por_usuario(user, admin):
    ...
    for aba, tarefas_periodo, titulo, data_limite in zip(
        ...
        with aba:
            ...
            num_tarefas_atrasadas = sum(1 for t in tarefas if t["status"] == "🟥 Atrasado" and t["Data de Execução"] < hoje)

            st.subheader(f"🟥 Atrasado - {titulo} ({num_tarefas_atrasadas})")

            # Filtrar tarefas atrasadas corretamente
            df_atrasadas = pd.DataFrame([t for t in tarefas if t["status"] == "🟥 Atrasado" and t["Data de Execução"] < hoje])
